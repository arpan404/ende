...
export default function Encrypt(props) {
  ...
  const handleAction = async () => {
    if (userInput.length > 0) {
      try {
        let encrypted = await CryptoJS.AES.encrypt(userInput, pwd);
        if (encrypted.toString().length > 0) {
          setMsg(encrypted.toString());
          setError(false);
        } else {
          setError(true);
  ...
  return (
    <div>
      <div className="lg:mt-2 lg:ml-4  mt-5 ml-5">
        ...
        <Link to="/decrypt">
          <b className="text-white ml-4 text-xl">Decryption</b>
        </Link>
      </div>
      ...
      {msg !== "" && <MsgBox type="encryption" msg={msg} isError={error} />}
    </div>
  );
}
